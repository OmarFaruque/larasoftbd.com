...
class SS_Public {
	...
	public function load_assets() 
	{
		...
		foreach ($opts['load_static'] as $static){
			if($static == 'load_css'){
				wp_enqueue_style( 'wp-social-sharing', get_template_directory_uri() . '/inc_function/social-sharing/static/socialshare.css', array(), SS_VERSION );
			}	
			if($static == 'load_js'){
				wp_enqueue_script( 'wp-social-sharing', get_template_directory_uri() . '/inc_function/social-sharing/static/socialshare.js', array(), SS_VERSION, true );				
			}		
		}
	}
	...
	public function social_sharing( $atts=array() ) {
		...
		<div class="social-sharing <?php echo $sssocial_sharing;?>">
                                    <div class="post-share-options">
                                        <div class="social-posts mb20 pull-right">
                                        <?php if(!empty($before_button_text) && ($text_position == 'left' || $text_position == 'top')):?>
                                            <strong class="<?php echo $text_position;?>"><?php echo $before_button_text; ?></strong>
                                            <?php endif;?>
                                            <?php 
                                            foreach($icon_order as $o) {
	        										...
	        										case 'f':
                                             ?>
												<a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo $ssbutton_facebook;?>" href="http://www.facebook.com/sharer/sharer.php?u=<?php echo $url; ?>" target="_blank" ><span class="<?php echo $facebook_text; ?>"></span></a>
                                             <?php break; 
                                             case 't':
								        			if(in_array('twitter', $social_options)){
								        			?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo $ssbutton_twitter;?>" href="http://twitter.com/intent/tweet/?text=<?php echo $title; ?>&url=<?php echo $url; ?><?php if(!empty($twitter_username)) {  echo '&via=' . $twitter_username; } ?>" target="_blank"><span class="<?php echo $twitter_text; ?>"></span></a><?php
								        			}
								        		break;
								        		case 'g':
								        			if(in_array('googleplus', $social_options)){
								        			?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo $ssbutton_googleplus;?>" href="https://plus.google.com/share?url=<?php echo $url; ?>" target="_blank" ><span class="<?php echo $googleplus_text; ?>"></span></a><?php
								        			}
								        		break;
												case 'l':
													if(in_array('linkedin', $social_options)){
														?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo $ssbutton_linkedin;?>" href="http://www.linkedin.com/shareArticle?mini=true&url=<?php echo substr($url,0,1024);?>&title=<?php echo substr($title,0,200);?>" target="_blank" ><span class="<?php echo $linkedin_text; ?>"></span></a><?php
													}
								        		break;
								        		case 'p':
								        			if(in_array('pinterest', $social_options)){
								        				?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo $ssbutton_pinterest;?>" href="http://pinterest.com/pin/create/button/?url=<?php echo $url;?>&media=<?php echo $social_image;?>&description=<?php echo $title;?>" target="_blank" ><span class="<?php echo $pinterest_text; ?>"></span></a><?php
								        			}
												break;
							                    case 'x':
							                        if(in_array('xing', $social_options)){
							                    	    ?><a <?php echo $loadjs;?> rel="external nofollow" class="<?php echo $ssbutton_xing;?>" href="https://www.xing.com/spi/shares/new?url=<?php echo $url;?>" target="_blank" ><span class="<?php echo $xing_text; ?>"></span></a><?php
							                        }
								        		break;  } }		?>
                                        </div>
                                    </div>
